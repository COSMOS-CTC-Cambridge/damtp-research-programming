A Small Addition to git
=======================

-   we've done
    -   set up a repo: `git init`
    -   add files to repo: `git add`
    -   commit changes: `git commit`
    -   give names to specifit commits: `git tag`
    -   create a parallel development line, a branch: `git branch`
    -   changed from branch to another: `git checkout`
    -   merged (combined) changes in two branches: `git merge`
-   what we still need to do
    -   update changes made to some else's repository
    -   make our commits available to others

Remote Repositories
-------------------

-   recall that `git init` creates a local repository where your collaborators have no access
-   you can arrange them to access that, but it is better to have a "bare" repository, i.e. a repository which has no working directory associated with it
-   a bare repo is usually placed on a server, it is almost "a central repo"
    -   it is central only in the sense that it is **a** designated "forum" to exchange commits with collaborators
    -   it holds no extra data and can be recreated from any up-to-date (local) clone
    -   there could be many of these bare repos for any one project (though usually not as it isn't very useful)
-   the word "remote" is slightly misleading: it only means the "remote" repository is not in the same directory as the "local"
    -   it can be on the same computer or it can be on the Moon (assuming there's internet there, of course), git does not care
-   when you did `git clone` of the repo for this course, you cloned a bare repo on a server
-   but I have made changes to it, so you now need to incorporate those to your clone!

### Incorporate Remote Changes to Local Clone

-   before you do a thing, make sure you are clean: `git status` and if necessary `git commit -a`
    -   in **this** and **only this** case do `git reset --hard` to undo any edits you may have made (jupyter will have autosaved your files!)
-   fetch the remote version: `git fetch origin master`
-   normally, at this point, you do `git diff FETCH_HEAD` and check what changes will be merged
    -   now you do not as the hard reset has removed all local changes so the merge will be a FF
-   finally, do `git merge origin/master`
    -   there's a shortcut which you should not normally use: fetch and merge can be combined to a single `git
             pull`
    -   for the hard resetted course repo, pull will work
...
-   Gives another **10x** or so over 2005 performance
    -   Xeon Phi gives **30x**
-   SIMD and multicore together give 320\$\\times\$/Hz (or 200x real) and Xeon Phi even 1000x!
...
-   Write smart, well designed code, optmimise on-core performance later
    -   But expect optimising will change the scaling characteristics (c.f. <fig:domain_decomp_strong_scaling> --- the initial ratio will change and usually to the worse)
...
-   write a code find the two lowest spectral lines of hydrogen (or, more precisely, find the two lowets eigenvalues of the Schr\\"odinger operator of a hydrogen atom)
-   take as many details as you wish in your Schr\\"odinger equation, but at minimum (\(\mu\) is the reduced mass)
